...
class Producer:
    ...
    def create_topic(self):
        ...
        if self.topic_name in list_of_topics.topics:
            logger.info(f"{self.topic_name} already exists. provide a different topic name TEST .")
            return
        
        topic_list = [NewTopic(topic = self.topic_name, num_partitions=3, replication_factor=1)]
        #logger.info(f"{topic_list} test.")
        ...
        for topic, future in futures.items():
            try:
                ...
                print(f"exception-{e} occured while creating this topic. topic creation kafka integration incomplete - skipping")
    ...
    #has been repeated for some reason therefore commented
    #def time_millis(self):
